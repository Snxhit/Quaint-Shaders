...
vec3 edgeDetectNether(sampler2D colortex0, sampler2D depthtex0, vec2 texcoord, float viewWidth, float viewHeight, int excludedBlockID) {
...
/*void edgeDetectOverworld() {
    ...
    if (gradientmagnitude > 0.01) {
        //below is the og one
        //color.rgb = mix(baseColor, outlineColor, edgeFactor * outlineAlpha);
        color.rgb = baseColor.rgb * EDGE_BRIGHTNESS;
    } else {
        color.rgb = texture(colortex0, texcoord).rgb;
    }
}*/
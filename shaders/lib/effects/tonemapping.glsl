...
vec3 applyToneMapping(int dimension, sampler2D colortex0, vec2 texcoord) {
    ...
    vec3 color = texture(colortex0, texcoord).rgb;
        ...
        if (dimension == 0) {
            color.rgb = ACESFilmOverworld(color.rgb);
        } else if (dimension == 1) {
            color.rgb = ACESFilmNether(color.rgb);
        } else if (dimension == 2) {
            color.rgb = ACESFilmOverworld(color.rgb);
        }
    #else
